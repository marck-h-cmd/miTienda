...
export default function ProductCard({ producto, viewMode = 'grid' }: ProductCardProps) {
  ...
  const imagenPrincipal = producto.cat_imagenes_producto?.find((img) => img.es_principal)?.url
  ...
  const handleAgregarCarrito = async (e: React.MouseEvent) => {
    ...
    try {
      await carritoService.agregar({ producto_id: producto.id, cantidad: 1 });
  ...
  if (viewMode === 'list') {
    return (
      <Link to={`/producto/${producto.id}`} className="flex border rounded-lg p-4 hover:shadow-md transition gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 mt-2">
            {tieneOferta && (
              <span className="text-sm text-gray-400 line-through">S/ {producto.precio_venta}</span>
            )}
  ...
  return (
    <Link to={`/producto/${producto.id}`} className="border rounded-lg overflow-hidden hover:shadow-lg transition group">
      <div className="relative">
        <img src={imagenPrincipal} alt={producto.nombre} className="w-full h-48 object-cover group-hover:scale-105 transition" />
        {tieneOferta && (
