...
async function main() {
  ...
  const permisosPorRol: Record<string, string[]> = {
    ...
    GERENTE_VENTAS: [
      ...
      'clientes_LEER', 'reportes_LEER', 'dashboard_LEER',
    ],
    GERENTE_INVENTARIO: [
    ...
    VENDEDOR: [
      ...
      'clientes_LEER', 'dashboard_LEER',
    ],
    CLIENTE: ['productos_LEER', 'ordenes_LEER', 'ordenes_CREAR'],
  };
  ...
  const usuarios = [
    { email: 'admin@mitienda.com',      nombre: 'Admin',    apellido: 'Sistema',    rol: 'ADMINISTRADOR' },
    { email: 'gerente@mitienda.com',    nombre: 'Gerente',  apellido: 'Ventas',     rol: 'GERENTE_VENTAS' },
    { email: 'inventario@mitienda.com', nombre: 'Gerente',  apellido: 'Inventario', rol: 'GERENTE_INVENTARIO' },
    { email: 'vendedor@mitienda.com',   nombre: 'Vendedor', apellido: 'Tienda',     rol: 'VENDEDOR' },
    { email: 'cliente@email.com',       nombre: 'Juan',     apellido: 'Pérez',      rol: 'CLIENTE' },
    { email: 'maria@email.com',         nombre: 'María',    apellido: 'García',     rol: 'CLIENTE' },
    { email: 'carlos@email.com',        nombre: 'Carlos',   apellido: 'López',      rol: 'CLIENTE' },
  ];
  for (const u of usuarios) {
    ...
    const usuario = await prisma.seg_usuarios.upsert({
      ...
      create: { ...data, password_hash: passwordHash, activo: true, email_verificado: true },
    });
    ...
    if (rol === 'CLIENTE') {
      await prisma.cli_clientes.upsert({
        ...
        create: { usuario_id: usuario.id, nombre: data.nombre, apellido: data.apellido, email: data.email },
      });
    }
  }
  ...
  // ==================== SUBCATEGORÍAS ====================
  // FIX: buscar por nombre+categoria_id en lugar de ID inventado
  const subcategorias = [
  ...
  for (const sub of subcategorias) {
    const categoriaId = categoriasCreadas[sub.categoria];
    const existe = await prisma.cat_subcategorias.findFirst({
      where: { nombre: sub.nombre, categoria_id: categoriaId },
    });
    if (!existe) {
      await prisma.cat_subcategorias.create({
        data: { nombre: sub.nombre, categoria_id: categoriaId },
      });
    }
  }
  ...
  // ==================== MARCAS ====================
  const marcas = ['Samsung', 'Apple', 'Sony', 'Nike', 'Adidas', 'LG', 'Xiaomi', 'Generic'];
  const marcasCreadas: Record<string, string> = {};
  for (const nombre of marcas) {
    const marca = await prisma.cat_marcas.upsert({
      where: { nombre },
      update: {},
      create: { nombre },
    });
    marcasCreadas[nombre] = marca.id;
  }
  ...
  for (const unidad of unidades) {
    const u = await prisma.cat_unidades_medida.upsert({
      where: { nombre: unidad.nombre },
      update: unidad,
      create: unidad,
    });
    unidadesCreadas[unidad.abreviatura] = u.id;
  }
  console.log('✅ Unidades de medida creadas');

  // ==================== MÉTODOS DE ENVÍO ====================
  // FIX: buscar por nombre en lugar de ID inventado
  const metodosEnvio = [
  ...
  for (const metodo of metodosEnvio) {
    const existe = await prisma.ord_metodos_envio.findFirst({
      where: { nombre: metodo.nombre },
    });
    const m = existe ?? await prisma.ord_metodos_envio.create({ data: metodo });
    metodosEnvioCreados[metodo.nombre] = m.id;
  }
  ...
  // ==================== PRODUCTOS ====================
  // FIX: upsert por sku en lugar de create ciego
  const productos = [
    {
      sku: 'SAM-S24-001', nombre: 'Samsung Galaxy S24',
      descripcion_corta: 'Smartphone de última generación',
      descripcion_larga: 'El Samsung Galaxy S24 cuenta con pantalla Dynamic AMOLED de 6.2", procesador Exynos 2400, 8GB RAM y 256GB.',
      categoria: 'Electrónica', marca: 'Samsung', unidad: 'UND',
      precio_costo: 2500.00, precio_venta: 3499.00, stock: 15, stock_minimo: 5,
    },
    {
      sku: 'APP-IP15-002', nombre: 'iPhone 15 Pro',
      ...
      categoria: 'Electrónica', marca: 'Apple', unidad: 'UND',
      precio_costo: 3800.00, precio_venta: 5299.00, precio_oferta: 4999.00, stock: 10, stock_minimo: 3,
    },
    {
      sku: 'SONY-WH1-003', nombre: 'Sony WH-1000XM5',
      descripcion_corta: 'Auriculares inalámbricos con cancelación de ruido',
      descripcion_larga: 'Auriculares over-ear con cancelación de ruido líder en la industria, 30 horas de batería.',
      categoria: 'Electrónica', marca: 'Sony', unidad: 'UND',
      precio_costo: 800.00, precio_venta: 1299.00, stock: 25, stock_minimo: 10,
    },
    {
      sku: 'NK-AIR-004', nombre: 'Nike Air Max 270',
      descripcion_corta: 'Zapatillas deportivas con amortiguación Air',
      descripcion_larga: 'Zapatillas Nike Air Max 270 con unidad Air de 270 grados y parte superior de malla transpirable.',
      categoria: 'Ropa', marca: 'Nike', unidad: 'PAR',
      precio_costo: 250.00, precio_venta: 499.00, stock: 40, stock_minimo: 10,
    },
    {
      sku: 'AD-ULTR-005', nombre: 'Adidas Ultraboost 23',
      ...
      categoria: 'Ropa', marca: 'Adidas', unidad: 'PAR',
      precio_costo: 300.00, precio_venta: 599.00, stock: 30, stock_minimo: 8,
    },
    {
      sku: 'LG-OLED-006', nombre: 'LG OLED TV 55" C3',
      descripcion_corta: 'Smart TV OLED 4K con IA',
      descripcion_larga: 'LG OLED C3 de 55" con procesador α9 Gen6 AI, Dolby Vision, Dolby Atmos y webOS 23.',
      categoria: 'Electrónica', marca: 'LG', unidad: 'UND',
      precio_costo: 3200.00, precio_venta: 4599.00, stock: 8, stock_minimo: 2,
    },
    {
      sku: 'XM-REDN-007', nombre: 'Xiaomi Redmi Note 13',
      descripcion_corta: 'Smartphone con cámara de 108MP',
      descripcion_larga: 'Xiaomi Redmi Note 13 con pantalla AMOLED de 6.67", cámara de 108MP, batería de 5000mAh.',
      categoria: 'Electrónica', marca: 'Xiaomi', unidad: 'UND',
      precio_costo: 600.00, precio_venta: 899.00, stock: 35, stock_minimo: 10,
    },
    {
      sku: 'GEN-TSH-008', nombre: 'Camiseta Básica Algodón',
      descripcion_corta: 'Camiseta 100% algodón premium',
      descripcion_larga: 'Camiseta de algodón peinado, cuello redondo, disponible en varios colores.',
      categoria: 'Ropa', marca: 'Generic', unidad: 'UND',
      precio_costo: 15.00, precio_venta: 39.90, stock: 200, stock_minimo: 30,
    },
    {
      sku: 'GEN-PAN-009', nombre: 'Pantalón Jean Clásico',
      descripcion_corta: 'Jean de mezclilla de alta calidad',
      descripcion_larga: 'Pantalón jean de mezclilla stretch, corte recto, disponible en azul y negro.',
      categoria: 'Ropa', marca: 'Generic', unidad: 'UND',
      precio_costo: 45.00, precio_venta: 99.90, stock: 150, stock_minimo: 25,
    },
    {
      sku: 'GEN-MUE-010', nombre: 'Escritorio de Oficina',
      ...
      categoria: 'Hogar', marca: 'Generic', unidad: 'UND',
      precio_costo: 180.00, precio_venta: 349.00, stock: 12, stock_minimo: 3,
    },
    {
      sku: 'GEN-DEC-011', nombre: 'Lámpara de Mesa LED',
      descripcion_corta: 'Lámpara LED regulable para escritorio',
      descripcion_larga: 'Lámpara LED con brazo ajustable, 3 modos de iluminación y puerto USB.',
      categoria: 'Hogar', marca: 'Generic', unidad: 'UND',
      precio_costo: 35.00, precio_venta: 79.90, stock: 60, stock_minimo: 15,
    },
    {
      sku: 'GEN-FUT-012', nombre: 'Balón de Fútbol Profesional',
      descripcion_corta: 'Balón oficial tamaño 5',
      descripcion_larga: 'Balón de fútbol cosido a máquina, cubierta de PU y cámara de butilo.',
      categoria: 'Deportes', marca: 'Generic', unidad: 'UND',
      precio_costo: 40.00, precio_venta: 89.90, stock: 45, stock_minimo: 10,
    },
    {
      sku: 'GEN-GYM-013', nombre: 'Set de Pesas 20kg',
      descripcion_corta: 'Set de mancuernas ajustables',
      descripcion_larga: 'Set de pesas con barra y discos, total 20kg. Incluye 4 discos de 2.5kg y 4 de 1.25kg.',
      categoria: 'Deportes', marca: 'Generic', unidad: 'UND',
      precio_costo: 80.00, precio_venta: 159.90, stock: 20, stock_minimo: 5,
    },
    {
      sku: 'GEN-FIC-014', nombre: 'Cien Años de Soledad',
      descripcion_corta: 'Obra maestra de Gabriel García Márquez',
      descripcion_larga: 'Edición conmemorativa del 50 aniversario. Tapa dura, 512 páginas.',
      categoria: 'Libros', marca: 'Generic', unidad: 'UND',
      precio_costo: 25.00, precio_venta: 59.90, stock: 80, stock_minimo: 20,
    },
    {
      sku: 'GEN-NOF-015', nombre: 'Hábitos Atómicos',
      ...
      categoria: 'Libros', marca: 'Generic', unidad: 'UND',
      precio_costo: 20.00, precio_venta: 49.90, stock: 100, stock_minimo: 25,
    },
    {
      sku: 'GEN-MUN-016', nombre: 'Set de Muñecas Princesas',
      descripcion_corta: 'Set de 4 muñecas princesas',
      descripcion_larga: 'Incluye 4 muñecas de 30cm con vestidos intercambiables y accesorios.',
      categoria: 'Juguetes', marca: 'Generic', unidad: 'UND',
      precio_costo: 30.00, precio_venta: 69.90, stock: 50, stock_minimo: 10,
    },
    {
      sku: 'GEN-JME-017', nombre: 'Juego de Mesa Monopoly',
      descripcion_corta: 'Clásico juego de negocios',
      descripcion_larga: 'Monopoly edición clásica con tablero, fichas metálicas, dinero y tarjetas.',
      categoria: 'Juguetes', marca: 'Generic', unidad: 'UND',
      precio_costo: 35.00, precio_venta: 79.90, stock: 35, stock_minimo: 8,
    },
    {
      sku: 'GEN-SNA-018', nombre: 'Mix de Frutos Secos 500g',
      ...
      categoria: 'Alimentos', marca: 'Generic', unidad: 'UND',
      precio_costo: 12.00, precio_venta: 24.90, stock: 120, stock_minimo: 20,
    },
    {
      sku: 'GEN-CFA-019', nombre: 'Crema Hidratante Facial SPF30',
      ...
      categoria: 'Belleza', marca: 'Generic', unidad: 'UND',
      precio_costo: 18.00, precio_venta: 44.90, stock: 90, stock_minimo: 15,
    },
    {
      sku: 'SAM-TAB-020', nombre: 'Samsung Galaxy Tab S9',
      descripcion_corta: 'Tablet Android premium con S Pen',
      descripcion_larga: 'Tablet de 11" Dynamic AMOLED 2X, Snapdragon 8 Gen 2, 12GB RAM, 256GB, S Pen incluido.',
      categoria: 'Electrónica', marca: 'Samsung', unidad: 'UND',
      precio_costo: 1800.00, precio_venta: 2699.00, precio_oferta: 2499.00, stock: 12, stock_minimo: 3,
    },
  ];
  ...
  for (const prod of productos) {
    ...
    // FIX: upsert por sku — idempotente en reruns
    const producto = await prisma.cat_productos.upsert({
      ...
      update: {
        ...
        precio_oferta: (prod as any).precio_oferta ?? null,
